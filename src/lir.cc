...
void LIR::SpillActive(HIRInstruction* hinstr) {
  // TODO: Insert length=1 fixed interval
}


void LIR::TranslateInstruction(HIRInstruction* hinstr) {
  int i;
  ...
  // (if that's possible)
  if (hinstr->HasSideEffects()) SpillActive(hinstr);
